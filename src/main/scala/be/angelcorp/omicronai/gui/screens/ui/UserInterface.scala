  ...
  def screen(nifty: Nifty, gui: AiGui) = {
    ...
    val xml =
      ...
      <nifty xmlns="http://nifty-gui.lessvoid.com/nifty-gui" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" >
        <screen id={name} controller={classOf[UserInterfaceScreenController].getName}>
    ...
    loadNiftyXml( nifty, xml, new UserInterfaceScreenController(gui) )
    nifty.getScreen( name )
  }

}

class UserInterfaceScreenController(gui: AiGui) extends ScreenController {
