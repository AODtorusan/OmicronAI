...
import be.angelcorp.omicron.base.{HexTile, HexTileEdge, RegionOfInterest}
...
class RegionRenderer(val roi:    RegionOfInterest,
  ...
  lazy val shapes = {
    ...
    val eps = 1E-4f
    val shapes = mutable.ListBuffer[Polygon]()
    while (segments.nonEmpty) {
      ...
      val points = mutable.ListBuffer[(Float, Float)]( endPoint )
