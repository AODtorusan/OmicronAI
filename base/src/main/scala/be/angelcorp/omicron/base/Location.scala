...
class Location( val u: Int, val v: Int, val h: Int, val bounds: WorldBounds ) {
  ...
  def spiral( maxRadius: Int, outStep: Int = 1, outDirection: Direction = E() ) = {
    ...
    tiles.flatMap( _.toPlain.reduce ).result()
  }
  ...
  def toPlain    = new Location(u, v, h, bounds)
  def toWrapping = new WrappingLocation(u, v, h, bounds)
  def toVirtual  = new VirtualLocation(u, v, h, bounds)
...
/**
 * A virtual location is a basic location, but not consrtained to the world bounds, and that does not wrap arround
 */
case class VirtualLocation( _u: Int, _v: Int, _h: Int, _bounds: WorldBounds ) extends Location(_u, _v, _h, _bounds) {

  override def Δ(δu: Int, δv: Int, δh: Int): Option[Location] =
    Some(new VirtualLocation(u + δu, v + δv, h + δh, bounds))

  override def Δ2(δu: Int, δv: Int): Option[Location] =
    Some(new VirtualLocation(u + δu, v + δv, h, bounds))

  override def reduce: Option[Location] = Some(this)
...
case class WrappingLocation( _u: Int, _v: Int, _h: Int, _bounds: WorldBounds ) extends Location(_u, _v, _h, _bounds) {
  ...
  override def reduce: Option[Location] = {
    ...
    Some(new WrappingLocation(newU, newV, h, bounds))
  }
